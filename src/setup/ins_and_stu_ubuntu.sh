...
then
    ...
    AS_DL_URL=$(curl "https://developer.android.com/studio/index.html#downloads" | grep -o -m 1 --regexp='https.*\.gz')
    ...
    echo -e "\n\n $Purple Downloading Android Studio for Ubuntu. $RESET \n\n"
    curl -o studio.tar.gz -L $STUDIO

    echo -e "\n\n $Yellow Installing Android Studio for Ubuntu.. $RESET \n\n"
    sudo tar -xzvf studio.tar.gz -C /opt

    sudo rm studio.tar.gz
